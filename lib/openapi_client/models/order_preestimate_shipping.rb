  ...
  class OrderPreestimateShipping
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OrderPreestimateShipping` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OrderPreestimateShipping`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'method_code')
        self.method_code = attributes[:'method_code']
      end
    ...
    def hash
      [method_code, method_name, carrier_code, carrier_name, description, price, price_inc_tax, delivery_time, additional_fields, custom_fields].hash
    end

  ...
  class ProductImageAdd
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ProductImageAdd` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ProductImageAdd`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'variant_ids')
        self.variant_ids = attributes[:'variant_ids']
      end

      if attributes.key?(:'option_value_ids')
        self.option_value_ids = attributes[:'option_value_ids']
      end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["small", "base", "additional", "thumbnail"])

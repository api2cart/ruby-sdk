  ...
  class OrderPreestimateShippingListOrderItemInner
    ...
    def self.openapi_types
      {
        ...
        :'order_item_option' => :'Array<OrderPreestimateShippingListOrderItemInnerOrderItemOptionInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OrderPreestimateShippingListOrderItemInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OrderPreestimateShippingListOrderItemInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [order_item_id, order_item_model, order_item_quantity, order_item_weight, order_item_variant_id, order_item_option].hash
    end
